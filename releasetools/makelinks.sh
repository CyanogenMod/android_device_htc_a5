...
if [ $variant == "dualsim" ]; then
	...
	NFCFILES="app/NfcNci etc/permissions/android.hardware.nfc.xml etc/permissions/android.hardware.nfc.hce.xml etc/permissions/com.android.nfc_extras.xml etc/permissions/com.cyanogenmod.nfc.enhanced.xml etc/nfcee_access.xml etc/libnfc-nxp.conf etc/libnfc-brcm.conf lib/libnfc-nci.so lib/hw/nfc_nci.msm8226.so lib/libnfc_nci_jni.so lib/libnfc_ndef.so framework/com.android.nfc_extras.jar priv-app/Tag.apk vendor/firmware/libpn547_fw.so"
